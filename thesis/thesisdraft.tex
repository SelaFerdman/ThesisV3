...
\section{Experimental setup}
We chose nineteen companies (see \autoref{table:companies_list}) among well-known addon distributors. There are a few "famous" companies that are distributing addons and toolbars.Their addons are distributed with free programs, like Adobe Reader, that include add-ons like toolbars or browser extensions. Most of the time, during the installation of the software, an opt-out option will be presented for the add-on. But, lots of people click through without reading, and when they’re finished they discover they downloaded something they didn’t intend to. Anti Virus and Anti Malware companies are on the other side of this domain (and sometimes on both sides). For example, Avast Anti Virus  published back in 2013 a list of top 10 companies that are distributing their addons this way (https://blog.avast.com/2013/03/20/avast-browser-cleanup-at-work/). Surprisingly, the list Avast company  published in 2015 is not very different and many of these companies are in our list. Some Anti Virus companies are also distributing toolbars in similar way. For example, AVG Anti Virus company has AVG Security Toolbar which is detected by Avast Anti Virus as malware. Back in 2013, Avast  indicated more than 3,300,000 different browser extensions for the three main browsers. They also noticed that "A lot of toolbars are available in different variants. These variations affect mostly the name".
In Avast recent blog post from July 9th, 2015, they desribe the addons ecosystem in user's computers, and one of its attributes is described as "The addons fight against each other..." (https://blog.avast.com/2015/07/09/top-10-most-annoying-browser-toolbars/).
Due to Avast statistics of toolbars removals, some companies from our list are at top 10, like Conduit with more than 13,000,000 removals, ASK with 11,000,000 removals and others with similar magnitude. Interestingly, the same companies that are claiming to fight against the addons distributor due to its distribution ways, are using the same techniques to distribute toolbars of other companies.(http://techdows.com/2012/11/avast-comes-bundled-with-google-toolbar.html) "Avast is contradicting itself. Their latest product offers a built-in feature to rid your browser of toolbars, while offering a toolbar when installing their software."
...
Our initial efforts were concentrated around addon survival prediction. We were trying to classify users that are staying long in user ecosystem and those that churn immediately, lately this would turn to Clash prediction.
We have defined training data as follows - positive examples are the users that have stayed at the system more than three days and others are negative example. We have used a popular libSVM tool to run the analysis.

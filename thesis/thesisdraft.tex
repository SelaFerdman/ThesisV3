...
{\it Representation of add-on path information.} Add-on
names often include full file-system path information, e.g.,
`C:/Program Files (x86)/...'. Supposedely, it can be harmful to link
add-ons to common {\it terms} associated with path information (e.g., `program';
`files'; `x86'). Such links may connect add-ons simply due to being installed in a similar system environment. We therefore remove common path
...
As the generation of query-dependent distributions requires computation of the Personalized PageRank per query, for multiple configurations

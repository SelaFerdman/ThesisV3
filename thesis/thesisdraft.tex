...
members of personalized vector $b$. In practice, given PageRank as in equation~(\ref{eq:pr})
			\begin{equation}
				...
				\label{eq:pr}
			\end{equation}
the PageRank equation is adapted as follows to compute the Personalized PageRank ~(\ref{eq:ppr}):
			\begin{equation}
				\overrightarrow{v} = \alpha A^{\rm T}\overrightarrow{v}
...
RELATED?: As mentioned before, the algorithms of PageRank and Personalized
...
\subsection{Applying PPR for finding the missing add-on}

CONTINUE HERE. In this research, the question that we wish to answer is `given
...
user's environment. We expect relatedness to flow over multiple path
...
Networks book \citep{ting2004minig}). Concretely, ARPACK applies an implicitly restarted Arnoldi method to calculate the random walk scores. \iffalse igraph expects as an adjacency list of numeric
...
We aggregate PageRank scores of all addons of a company into the expected score and compare companies based on the expected score. For each company $c_q$ in the PPR query, we construct a ranked list of all other companies according to their expected PPR scores. In addition, we maintain a ranked list of companies according to their expected (non-personalized) PR scores (see~\autoref{table:pagerank_scores}). For each $c_q$, we compare the two ranked lists of companies. If a company $c_i$ ``pops up'' closer to the top of the $c_q$'s PPR ranked list of companies, as compared to the original (non-personalized) ranked list of companies, we conclude that $c_i$ and $c_q$ are in a symbiotic relationship. 
...
\centering
    ...
    \caption{Ratios of Expected PPR and expected PR of companies}

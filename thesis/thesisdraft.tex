...
[RON: the term ``computer virus'' has existed for a while - the connection between a hardware system (a computer) with a biological system is quite intuitive. However, we go far beyond this - we're investigating biological _processes_ in an artificial environment, which sounds pretty novel]
...
[RON: we need a paragraph here that explains how it works. It should be something like that: In previous chapters (cite) we applied a PPR-like algorithm for... The algorithm took a list of addons and reranked them according to the query that was... We can apply a similar technique here for figuring out which addon belongs to which company. We start with queries that are... and apply PPR on them. Our teleportation vector... We see that some addons pop up significantly in the ranked list. We consider those addons to belong to the company in the query otherwise it would be no reason for them to pop up. We validate this hypothesis manually on those addons that popped up. On a validation set consisting of ... addons, we see 100\% precision meaning that there was not one single addon that would pop up in the ranked list and did not after all belong to the company in the query. Using this procedure, we found ... addons like that. WE NEED A LITTLE MORE MEAT IN THIS PARAGRAPH] %as the teleportation vector [SELA:] we provide the previously found addons that are related to the specific company.
[SELA:]
It previous ~\autoref{chap:user_ecosystem} we applied a Personalized PageRank based algorithm for detecting an addon in specific browser(user) environment, that algorithm used a list of known user addons as a query input to PPR, the output of the algorithm is a ranked list of all addons in the graph. We've hypothesised that the missing addon would be very high in the ranked list, this hypothesis was proved correct.
We can apply a similar technique here, for mapping addons to their companies. Similarly to ~\autoref{chap:user_ecosystem}, we need a query of addons for the algorithm, in this case the input addons would be the known company addons (similarly to known user addons previously), we hypothesise that an addons that belong to that specific company, and are not in the query, would "pop-up" in the ranked list. By "pop-up" we are meaning that addon would be ranked high relatively to its previous rank in PageRank. Before running this algorithm, we ran a regular (not personalized) PageRank on the entire graph, we use the rank of addon after PageRank as the position for detecting if the addon drastically changed its position in the ranking. For example, if the addon was in rank 15 in regular PageRank and now it is in rank 14, we do not examining it since its rank has not dramatically changed. But if this addon was in rank 100 in regular PageRank that we will examine its company origin manually.
For each addon that "popped-up" in the ranking list and was not in the query before, we suspect that this addon is somehow related to the company. We validate this hypothesis manually on those addons that popped up. This process was executed iteratively, once we discovered a new addons in the first iteration (all addons that "popped-up"), we added them to the query and rerun the PPR with the extended query. The iteration process was stopped after there were no more addons that dramatically changed in its rank.
On a validation set consisting of 24 [SELA: Recheck the number] addons, we see 100\% precision meaning that there was not one single addon that would pop up in the ranked list and did not after all belong to the company in the query. Using this procedure, we found 24 addons like that.
...
For every add-on that was detected as suspected by the algorithm \autoref{alg:find_addon_species}, we've performed manual check to which company it belongs. This procedure has identified many $unknown$ addons and correctly related to the manufacture company. This outcome presents an interesting phenomena, there is some kind of community between addon of the same species, and this community is discovered by Personalized PageRank weight propagation from already known community members. [RON: any way to explain this phenomenon?]
[SELA:]
One possible explanation would be that it is a common practice to install a few addons of the same company - if it is already getting installed on the same machine. For example, if Babylon is installing an Internet Explorer addon, it might also install a Chrome extension. While Internet Explorer addon was mapped previously to Babylon company (due to name attribute, for example), the Chrome extension might not contain any attribute that would qualify it be related to Babylon but our algorithm would discover it in a similar way that it has discovered a missing addon for a specific user. This means that we are discovering all the modules that are related to a specific company that is installed on the machine, given only a few known addons of that company.
Another similar to the above explanation, is that even of not the same user but the users that are getting the company software installed, have in common many addons.

Now that we linked addons with their species (that is, their development companies), we are ready to analyze relations between the addon species in World Wide Web ecosystem. The following algorithm~\autoref{alg:collect_addon_data} summarizes our methodology. [RON: we still need to describe it in plain English, please add a paragraph of explanation] %the collection of all necessary data for each addon and its company.

[SELA:]
After we have collected all the addons that are related to every company, we will execute the algorithm describe in ~\autoref{chap:user_ecosystem}, as a query we will pass all company addons. The output is the ranked list of all addons in the graph. We perform this procedure for every company.
While the addons of the query are obviously highly ranked we have made another interesting observations about relations between the companies, as described in \autoref{sec:symb_relations}
...
The ordinary (non-personalized) PageRank score that each addon describes an ecosystem where non of the species are biased. This viewpoint allows us to assume what is the original species distribution when no other species is helping or disturbing the other. The total PageRank scores for each company is shown in \autoref{table:pagerank_scores}. [RON: tried to understand but couldn't - need to discuss this in person]
[SELA: I am trying to explain here our baseline, which is the regular PageRank for each addon, not biased - means the addons are not in query for PPR, I am saying that the rank in regular PageRank is showing how the addons are distributed when no connection to other company is made]
Add formula that describes relative jump
[SELA:]
We have looked at the top 100 ranked addons.
We have examined the addons that were not in top 100 and "jumped" into the top 100 ranked addons.

The story(as what I've described in our meeting):
After we have developed an algorithm that looked for missing addons in user environment, we noticed that there are addons that tend to appear with some addons while other addons almost never appear with them.
We have looked into those addons and discovered that there are specific companies that are affected by this behavior.
We decided to apply our algorithm to these companies.
The algorithm is described in ~\autoref{chap:user_ecosystem} but here is a brief description. Personalized PageRank (PPR) receives as an input seed nodes which are used as the query vector when computing the PPR. In previous sections the seed nodes where the user addons, now we want to do something similar with company addons. For that, firstly, we need to find each company addons. We identified the mapping of addon to company by looking at addon attributes and checking if company name appears there. After that we've looked for another addons that belong to that company by running the PPR with already known addons as the query and looking for addons that pop-up. This look-up procedure was proceeded until convergence.
After collecting addons for each company, we can run our PPR algorithm with query containing the company addons. The output is the ranked list of all addons. Obviously, the company addons are highly ranked, for measuring other addons fluctuation we removed the company addons from the ranked list. Now, once we have a new ranked list, we have examined the top 100 addons, we have compared their rank to the original addon PageRank. The original PageRank rank is the rank that this addon have after PageRank on the whole graph. 
We observed interesting relations between companies addons, such as that given one company addons, others company addons are also tend to move higher in the rank list while other companies addons move down.
